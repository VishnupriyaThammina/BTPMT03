...
plot(data.shape)
# new=read.csv("D:\\HONOURS PROJECT\\RESULTS\\GRAPHS\\PREDICTED_24\\hierarchical_24.csv",header = TRUE)
new=read.csv("D:\\HONOURS PROJECT\\RESULTS\\GRAPHS\\PREDICTED_16\\hierarchical_16.csv",header = TRUE)
# new=read.csv("D:\\HONOURS PROJECT\\RESULTS\\GRAPHS\\PREDICTED_8\\hierarchical_8.csv",header = TRUE)
...
# map<-ggplot(st,aes(V2,V1))+geom_raster(aes(fill=u))+geom_polygon(data=data.shape,aes(x=long,y=lat,group=group),color="black",fill=NA)+scale_fill_gradientn(name = "RMSE",colours=terrain.colors(10),breaks=c(10,15,20,24,28,32,36,40),limits=c(10,40))

# map<-ggplot(st,aes(V2,V1))+geom_raster(aes(fill=u))+geom_polygon(data=data.shape,aes(x=long,y=lat,group=group),color="black",fill=NA)+scale_fill_gradientn(name = "MAE",colours=terrain.colors(10))
