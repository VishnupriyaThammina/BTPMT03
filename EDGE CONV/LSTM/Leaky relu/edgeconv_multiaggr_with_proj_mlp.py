# -*- coding: utf-8 -*-
"""edgeconv model1 : BTP_WITH_AOD.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1c-xdM-3DhK3_nEp2XLZqKTtmo6xO6Kmn
"""
...
class EdgeConv(MessagePassing):
    def __init__(self, in_channels, out_channels):
        super().__init__(aggr=aggr.MultiAggregation(['mean', 'min', 'max'])) #  "Max" aggregation.
...
class PM25_GNN(nn.Module):
    ...
    def forward(self, pm25_hist, feature):
        ...
        for i in range(self.hist_len):
            ...
            xn_gnn = xn_gnn.view(self.batch_size, self.city_num, -1)
            print(xn_gnn.shape)
            edge_xn = self.mlp1(self.edge_conv(x, edge_index))
...
mean_score = np.mean(scores, axis=1)  # Mean score for each student
std_dev = np.std(scores, axis=1)      # Standard deviation for each student
max_score = np.max(scores, axis=1)    # Maximum score for each student

# Print the results
for i, (mean, std, max_) in enumerate(zip(mean_score, std_dev, max_score)):
    print(f"Student {i + 1}: Mean Score={mean}, Std Deviation={std}, Max Score={max_}")