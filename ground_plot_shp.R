...
new=read.csv("D:\\HONOURS PROJECT\\RESULTS\\GRAPHS\\PREDICTED_16\\ground_16.csv",header = TRUE)
# new=read.csv("D:\\HONOURS PROJECT\\RESULTS\\GRAPHS\\PREDICTED_8\\ground_8.csv",header = TRUE)
...
map<-ggplot(st,aes(V2,V1))+geom_raster(aes(fill=u))+geom_polygon(data=data.shape,aes(x=long,y=lat,group=group),color="black",fill=NA)+scale_fill_gradientn(name = "RMSE",colours=c("blue","green","violet","red"),breaks=c(9,17,24,30),limits=c(9,32),)
...
#   scale_colour_gradient2(name = "RSCORE",limits=c(0,1),low="blue",mid="green",high="red",breaks=c(0,0.3,0.6,0.9),space="Lab",guide = "colourbar")+
#   theme_void()
# 
#   
# map
