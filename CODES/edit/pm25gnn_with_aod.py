...
# in_dim_temp_gru=32
# in_dim_gnn_gru=25
results_dir = "/home/mainak.iiits/Indiagnn/results"
dataset_num = 1
# exp_model = 'PM25_GNN'
# model= PM25_GNN
...
proj_dir = os.path.dirname(os.path.abspath("/home/mainak.iiits/Indiagnn/"))
...
proj_dir = os.path.dirname(os.path.abspath("/home/mainak.iiits/Indiagnn/"))
sys.path.append(proj_dir)
proj_dir1 = "/home/mainak.iiits/Indiagnn/"
dem = genfromtxt("/home/mainak.iiits/Indiagnn/dataset/dem.csv", delimiter=",")
lcss = genfromtxt(
    "/home/mainak.iiits/Indiagnn/dataset/lcss.csv", delimiter=",")
...
city_fp = os.path.join(
    proj_dir1, "/home/mainak.iiits/Indiagnn/dataset/st_lulcIndia.csv"
)
...
totaldata = np.load(
    "/home/mainak.iiits/Indiagnn/dataset/Dataset.npy", allow_pickle=True
)
...
class HazeData(data.Dataset):
    def __init__(self, graph, hist_len=16, pred_len=8, dataset_num=1, flag="Train"):
    ...
    def _gen_time_arr(self):
        ...
        for time_arrow in arrow.Arrow.interval(
            "hours", self.data_start, self.data_end.shift(hours=+3), 3
        ):
...
train_data.feature = (
    train_data.feature - train_data.feature_mean
) / train_data.feature_std
train_data.pm25 = (train_data.pm25 - train_data.pm25_mean) / \
    train_data.pm25_std
test_data.feature = (
    test_data.feature - train_data.feature_mean
) / train_data.feature_std
...
class GraphGNN(nn.Module):
    def __init__(
        self, device, edge_index, edge_attr, in_dim, out_dim, wind_mean, wind_std
    ):
        ...
        self.edge_attr_norm = (
            self.edge_attr - self.edge_attr.mean(dim=0)
        ) / self.edge_attr.std(dim=0)
    ...
    def forward(self, x):
        ...
        src_wind = (
            node_src[:, :, -4:-2] * self.wind_std[None, None, :]
            + self.wind_mean[None, None, :]
        )
        ...
        out = torch.cat(
            [node_src, node_target, edge_attr_norm,
...
class GC_LSTM(nn.Module):
    def __init__(
        self, hist_len, pred_len, in_dim, city_num, batch_size, device, edge_index
    ):
        super(GC_LSTM, self).__init__()
        self.edge_index = torch.LongTensor(edge_index)
        self.edge_index = (
            self.edge_index.view(2, 1, -1).repeat(1, batch_size, 1)
...
def test(test_loader, model):
    ...
    for batch_idx, data in enumerate(test_loader):
        ...
        pm25_pred_val = (
            np.concatenate(
                [pm25_hist.cpu().detach().numpy(), pm25_pred.cpu().detach().numpy()],
                axis=1,
            )
            * pm25_std
            + pm25_mean
...
def main():
    ...
    for exp_idx in range(exp_repeat):
        ...
        logger = get_logger(results_dir + "%02d" %
                            exp_idx + "logger_SAGNN.log")
        ...
        for epoch in range(epochs):
            ...
            print(
                "Train loss: %0.4f, Test loss: %0.4f, RMSE: %0.2f,NRMSE: %0.4f, MAE: %0.2f,R2: %0.4f, CSI: %0.4f, POD: %0.4f, FAR: %0.4f"
